...
<a href="https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fcalcom%2Fcal.com&env=DATABASE_URL,NEXT_PUBLIC_WEBAPP_URL,NEXTAUTH_URL,NEXTAUTH_SECRET,CRON_API_KEY,CALENDSO_ENCRYPTION_KEY&envDescription=See%20all%20available%20env%20vars&envLink=https%3A%2F%2Fgithub.com%2Fcalcom%2Fcal.com%2Fblob%2Fmain%2F.env.example&project-name=cal&repo-name=cal.com&build-command=cd%20../..%20%26%26%20bun run%20build&root-directory=apps%2Fweb%2F" target="_blank">
...
<Steps>
    ...
    <Step title="Install packages with bun">
        ```
        bun install
        ```
    </Step>
...
<Warning>
If bun install fails during deployment on Vercel, ensure your bun.lock file is committed and try using ```bun install --frozen-lockfile``` as the install command.
</Warning>
