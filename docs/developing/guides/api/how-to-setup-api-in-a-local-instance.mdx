...
<Steps>
    ...
    <Step title="Start development server and create API key">
        Start the cal.com server using `bun run dev` on localhost and create the test API keys by visiting `/settings/developer/api-keys`
    </Step>
    ...
    <Step title="Start API server and test API">
        Start the API server by running `bun --filter @calcom/api run dev` and start testing your API locally
    </Step>
</Steps>
