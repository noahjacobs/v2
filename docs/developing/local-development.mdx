...
> For email testing, set it to "1" if you need to email checks in E2E tests locally. Make sure to run mailhog container manually or with `bun run dx`.
...
```bash
bun run playwright show-report test-results/reports/playwright-html-report
```
...
## **Development quick start with** **`bun run dx`**
