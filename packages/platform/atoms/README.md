...
- Run `bun run changesets-add` from monorepo root and then select `@calcom/atoms` using space bar and press enter to go to the next step.
...
1. Go to "packages/platform/atoms" and run `bun run dev-on` and then `bun run build` - this will create local build of atoms.
...
ORGANIZATION_ID=""
```
3. Go to "packages/platform/examples/base" and run `bun run dev:e2e` - this will start the example platform app and run e2e tests by using locally built atoms. Because it is not running within CI it will open a browser and run tests.
