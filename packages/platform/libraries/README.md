...
1. If you change platform libraries for the first time, then run `bun run local` to build them locally for the first time. This will also make v2 api point to the local libraries.
2. If you change them for the second time, then run `bun run build:dev` to re-build them.
3. Once you are happy with platform libraries:
- run `bun run publish-npm` - it will check "@calcom/platform-libraries" version in npm and update it's package.json to the next version and then it will publish the package to npm, update the version of "@calcom/platform-libraries" in the api v2 package.json, reset "@calcom/platform-libraries" to 0.0.0 and run bun install.
...
- Publish Your Version of Libraries on NPM:
  ...
  - Run bun run publish-npm to publish your version.
