...
1. `bun run changeset` -> Creates changelog files and adds summary to changelog. Select embed packages only here.
2. `bun run changeset version` -> Bumps the versions as required
3. Get the PR reviewed and merged
4. `bun run publish-embed` -> Releases all packages. We can't use `bun run changeset publish` because it doesn't support workspace: prefix removal yet. See https://github.com/changesets/changesets/issues/432#issuecomment-1016365428
